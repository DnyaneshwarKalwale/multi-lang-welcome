...
import { SunIcon, ArrowLeft, Twitter, Sparkles, Stars, CloudSun } from "lucide-react";
...
export default function ThemeSelectionPage() {
  ...
  // Set onboarding theme to light
  const handleThemeChange = () => {
    // Update onboarding state to light
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10 bg-gradient-to-br from-white to-primary-50 text-foreground relative overflow-hidden transition-colors duration-300">
      {/* Background decoration with floating elements */}
      <div className="absolute inset-0 -z-10 opacity-30">
        <div className="absolute top-0 -left-[30%] w-[70%] h-[70%] rounded-full bg-gradient-to-br from-primary-200 to-primary-300 blur-[120px]"></div>
        <div className="absolute bottom-0 -right-[30%] w-[70%] h-[70%] rounded-full bg-gradient-to-br from-violet-200 to-violet-300 blur-[120px]"></div>
      ...
      <motion.div 
        className="absolute top-[15%] right-[10%] text-primary-300 opacity-60 z-0"
      ...
      <motion.div 
        className="absolute bottom-[20%] left-[15%] text-violet-300 opacity-50 z-0"
        variants={floatingVariants}
        animate="animate"
      ...
      <motion.div 
        className="absolute top-[40%] left-[8%] text-cyan-300 opacity-40 z-0"
        variants={floatingVariants}
        animate="animate"
      ...
      >
        <Button
          variant="ghost"
          size="icon"
          className="w-10 h-10 text-gray-700 hover:bg-primary-100/50 hover:text-primary-500 rounded-full"
          onClick={prevStep}
        >
          <ArrowLeft size={18} />
        </Button>
      ...
      >
        ...
        <motion.h1 
          className="text-3xl md:text-4xl font-bold mb-6 bg-gradient-to-r from-primary-600 to-violet-600 bg-clip-text text-transparent"
          variants={itemVariants}
        >
          {t('chooseStyle')}
        </motion.h1>
        ...
        <motion.p 
          className="text-xl text-gray-700 mb-10 max-w-xl mx-auto"
          variants={itemVariants}
        >
          {t('styleDescription')}
        </motion.p>
        ...
        <motion.div 
          className="grid grid-cols-1 md:grid-cols-1 gap-6 mb-12 max-w-2xl mx-auto"
          variants={itemVariants}
        >
          <motion.div 
            className="ring-2 ring-primary-500 relative bg-white p-6 rounded-xl border border-gray-200 shadow-md cursor-pointer transition-all group overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-center">
              <div className="w-5 h-5 rounded-full border-2 border-primary-500 mr-3 flex items-center justify-center bg-primary-500">
                <div className="w-2 h-2 rounded-full bg-white"></div>
              </div>
              <span className="text-lg font-medium text-primary-500">{t('light')}</span>
            </div>
