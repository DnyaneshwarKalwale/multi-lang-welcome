...
const RequestCarouselPage: React.FC = () => {
  ...
  // Handle post selection modal result
  const handlePostSelectionApply = (content: string, counts: { twitter: number; linkedin: number }) => {
    setAttachedLinkedInPost(content);
    setShowPostSelectionModal(false);
    
    // Update the selected posts count for UI display
    setSelectedPostsForStyle({
      twitter: new Set(Array.from({ length: counts.twitter }, (_, i) => `twitter-${i}`)),
      linkedin: new Set(Array.from({ length: counts.linkedin }, (_, i) => `linkedin-${i}`))
    });
  };
  ...
  const handleSubscribe = () => {
    ...
    <div className="container max-w-6xl py-4 sm:py-8 px-1 sm:px-2 md:px-4 w-full overflow-hidden">
      ...
      <PostSelectionModal
        ...
        onApply={handlePostSelectionApply}
        ...
        initialSelectedPosts={selectedPostsForStyle}
      />
