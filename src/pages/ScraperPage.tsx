  ...
  const fetchTranscript = async (videoId: string) => {
    try {
      ...
      try {
        ...
        // If the endpoint is not found (404) or unauthorized (401), use a fallback method
        if (apiError.response && (apiError.response.status === 404 || apiError.response.status === 401)) {
          toast.error('Transcript API not available. Using video ID only.');
          
          // Create a placeholder transcript with the video ID
          const placeholderTranscript = `Video ID: ${videoId}\n\nThis is a placeholder transcript as the API is not available. In a production environment, this would be replaced with the actual transcript from the YouTube video.`;
  ...
  const handleGetTranscriptForVideo = async (videoId: string) => {
    ...
    try {
      ...
      if (transcript) {
        ...
        if (transcript.startsWith('Video ID:')) {
          // This is a fallback transcript
          toast.success(`Using placeholder transcript for video ID: ${videoId}`, {
            description: "The API for fetching real transcripts is not available. This is a placeholder."
          });
        } else {
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-md bg-yellow-50 p-4 border border-yellow-200">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-yellow-800">Transcript API Notice</h3>
                <div className="mt-2 text-sm text-yellow-700">
                  <p>
                    The YouTube transcript API is not currently available. When you click "Get Transcript", a placeholder will be provided instead of the actual video transcript. This will still allow you to create carousels, but they will not contain the real content from the videos.
                  </p>
                </div>
              </div>
            </div>
              ...
              >
                <div className="h-40 bg-gray-100 dark:bg-gray-800 relative">
                  ...
                  {videoTranscripts[video.id] && (
                    <div className={`absolute top-2 right-2 ${videoTranscripts[video.id].startsWith('Video ID:') ? 'bg-yellow-500/80' : 'bg-green-500/80'} text-white text-xs px-2 py-1 rounded-full flex items-center gap-1`}>
                      <FileText className="h-3 w-3" />
                      {videoTranscripts[video.id].startsWith('Video ID:') ? 'Placeholder' : 'Transcript Ready'}
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex space-x-1">
                    ...
                    >
                      {isTranscriptFetching[video.id] ? (
                        ...
                        <>
                          <FileText className={`h-4 w-4 mr-1 ${videoTranscripts[video.id].startsWith('Video ID:') ? 'text-yellow-500' : 'text-green-500'}`} />
                          {videoTranscripts[video.id].startsWith('Video ID:') ? 'View Placeholder' : 'View Transcript'}
                        </>
                      ) : (
                        <>
