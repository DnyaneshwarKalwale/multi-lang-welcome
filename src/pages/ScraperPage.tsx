...
const ScraperPage: React.FC = () => {
  ...
  const handleScrape = async () => {
    ...
    try {
      if (activeTab === 'twitter') {
        ...
        // Check if input is a channel or a video
  ...
  // YouTube Channel specific scraping logic
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const formattedVideos: YouTubeVideo[] = videos.map((video: any) => ({
          ...
          upload_date: video.upload_date || 'Unknown date',
          uploader: video.uploader || 'Unknown',
          channel_id: video.channel_id || '',
          description: video.description || '',
          webpage_url: video.webpage_url || `https://www.youtube.com/watch?v=${video.id}`
        }));
        
        setYoutubeVideos(formattedVideos);
        toast.success(`Successfully retrieved ${formattedVideos.length} videos`);
      } else {
  ...
  const handleVideoSelect = async (video: YouTubeVideo) => {
    ...
    // Fetch transcript for the selected video
    try {
      setIsLoading(true);
      await handleYouTubeScrape(video.webpage_url);
    } catch (error) {
      console.error('Error fetching transcript for selected video:', error);
      toast.error('Failed to fetch transcript for the selected video');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleYouTubeScrape = async (videoUrl?: string) => {
    try {
      if (!videoUrl && !inputUrl.includes('youtube.com') && !inputUrl.includes('youtu.be')) {
      ...
      if (response.data && response.data.success) {
        ...
        // Save to carousel request
        if (selectedVideo) {
          // Save the selected video info to state or session for CarouselsPage
          try {
            const savedItems = JSON.parse(localStorage.getItem('youtubeCarouselItems') || '[]');
            ...
            const newItem = {
              ...
              sourceUrl: selectedVideo.webpage_url
            };
            
            savedItems.push(newItem);
            localStorage.setItem('youtubeCarouselItems', JSON.stringify(savedItems));
  ...
  return (
    <div className="container mx-auto px-4 py-8">
              ...
              <div className="flex-1">
                <Input
                  ...
                  placeholder={
                    ...
                    activeTab === 'youtube' ? 'Enter YouTube video URL or channel name/URL' :
                    'Enter website URL'
                  }
                  value={inputUrl}
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    {activeTab === 'twitter' ? 'Fetch Tweets' : 
                     activeTab === 'youtube' ? 'Get Videos/Transcript' : 'Scrape Content'}
                  </>
                )}
              </Button>
            </div>
      ...
      {/* YouTube Channel Results Section */}
        ...
        <div className="mb-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {youtubeVideos[0]?.channel || 'Channel'} - {youtubeVideos.length} videos found
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {youtubeVideos.map((video) => (
                  <div 
                    ...
                    className={`border rounded-md overflow-hidden cursor-pointer transition-all hover:shadow-md ${
                      selectedVideo?.id === video.id ? 'border-primary ring-2 ring-primary/20' : 'border-gray-200'
                    }`}
                        ...
                        <span>
                          {typeof video.view_count === 'number' 
                            ? `${(video.view_count > 1000 
                                ? Math.floor(video.view_count / 1000) + 'K' 
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="flex justify-between mt-4">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => navigate('/dashboard/carousels')}
                      className="gap-2"
                    >
      ...
      {!result && !isLoading && !youtubeTranscript && youtubeVideos.length === 0 && !twitterResult && (
        <Card>
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                {activeTab === 'linkedin' 
                  ...
                  : activeTab === 'website'
                    ...
                    : 'Paste a YouTube video URL to extract transcripts, or a channel name/URL to browse videos. Create carousels from YouTube content.'
                }
              </p>
      ...
      {(isScrapingChannel) && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col items-center">
            ...
            <p className="text-lg font-medium">Fetching YouTube Channel...</p>
