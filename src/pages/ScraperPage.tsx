...
const ScraperPage: React.FC = () => {
  ...
  // Save YouTube content as carousel
  const handleSaveAsCarousel = async () => {
    if (!youtubeTranscript || !youtubeTranscript.videoId || !youtubeTranscript.transcript) {
      toast.error('No transcript available to save');
      return;
    }
    
    if (!videoTitle.trim()) {
      toast.error('Please enter a title for the carousel');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(apiUrl, {
        ...
        transcript: youtubeTranscript.transcript,
        title: videoTitle,
        generatedContent: linkedinContent,
        preferences: contentPreferences
      }, {
        headers: {
      ...
      if (response.data && response.data.success) {
        toast.success('Content saved as carousel successfully!');
        // Navigate to carousels page
        navigate('/dashboard/carousels');
      } else {
        throw new Error(response.data?.message || 'Failed to save carousel');
      }
    } catch (error) {
      console.error('Error saving as carousel:', error);
      toast.error('Failed to save as carousel. Please try again.');
    } finally {
  ...
  const getVideoTitle = async (videoId: string) => {
    try {
      const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/youtube/video-info`;
      const response = await axios.get(apiUrl, {
        params: { videoId },
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
  ...
  // Update to fetch title when transcript is loaded
  useEffect(() => {
    if (youtubeTranscript && youtubeTranscript.videoId) {
      getVideoTitle(youtubeTranscript.videoId);
    }
  }, [youtubeTranscript]);
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium mb-1">Carousel Title</label>
                      <Input
                        value={videoTitle}
                    ...
                    <Button 
                      variant="default"
                      onClick={handleSaveAsCarousel}
                      disabled={isSavingCarousel || !videoTitle.trim()}
