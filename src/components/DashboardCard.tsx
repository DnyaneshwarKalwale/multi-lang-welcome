...
export function DashboardPostCard({
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center">
          <img 
            src={author.avatar} 
            alt={author.name} 
            className="w-10 h-10 rounded-full mr-3 object-cover ring-2 ring-teal-400/30" 
          />
          <div>
        ...
        <div className="flex space-x-1">
          {onEdit && (
            <Button 
              variant="ghost" 
              size="icon" 
              className="h-8 w-8 rounded-full text-gray-500 hover:text-teal-600 hover:bg-gray-100 transition-all duration-200"
              onClick={onEdit}
            >
          ...
          {onDelete && (
            <Button 
              variant="ghost" 
              size="icon" 
              className="h-8 w-8 rounded-full text-gray-500 hover:text-red-500 hover:bg-gray-100 transition-all duration-200"
              onClick={onDelete}
            >
      ...
      <h3 className="text-lg font-semibold mb-2 line-clamp-1 text-gray-900">{title}</h3>
      ...
      {stats && (
        <div className="flex items-center justify-between mb-4 py-2 border-t border-gray-100">
          {stats.views !== undefined && (
            <div className="flex items-center text-xs text-gray-500 hover:text-teal-600 transition-colors duration-200">
    ...
    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center text-sm">
          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${increase > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
      ...
      <div className="h-32 relative">
        ...
        <div className="absolute inset-0 flex items-end">
          {data.map((value, index) => (
            ...
            >
              <div 
                className="w-full h-full rounded-t-sm bg-gradient-to-t from-cyan-500/40 to-teal-400/40"
      ...
      <div className="flex justify-between items-center mt-4 px-1 text-xs text-gray-500">
    ...
    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
      <div className="flex items-center mb-5">
        <img 
          src={user.avatar} 
          alt={user.name} 
          className="w-10 h-10 rounded-full object-cover mr-3 ring-2 ring-teal-400/30"
        />
        <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-5">
        <div className="bg-gray-50 p-3 rounded-lg text-center">
          <p className="text-lg font-semibold text-gray-900">{stats.posts}</p>
          <p className="text-xs text-gray-500">Posts</p>
        </div>
        <div className="bg-gray-50 p-3 rounded-lg text-center">
          <p className="text-lg font-semibold text-gray-900">{stats.followers.toLocaleString()}</p>
          <p className="text-xs text-gray-500">Followers</p>
        </div>
        <div className="bg-gray-50 p-3 rounded-lg text-center">
          <p className="text-lg font-semibold text-gray-900">{stats.views.toLocaleString()}</p>
      ...
      <div className="flex justify-between items-center pt-3 border-t border-gray-100">
        ...
        <p className="text-xs font-medium text-teal-600">85%</p>
      </div>
