...
import { Loader2, MessageSquare, ChevronLeft, Check, Folder, Twitter, Linkedin, ArrowLeft, X } from "lucide-react";
...
interface PostSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (selectedContent: string, selectedCounts: { twitter: number; linkedin: number }) => void;
  title?: string;
  description?: string;
...
// Create a separate component for LinkedIn post content to avoid hooks issues
const LinkedInPostContent: React.FC<{ content: string }> = ({ content }) => {
  ...
  return content.length > 250 && !isExpanded ? (
    ...
    <p className="whitespace-pre-line break-words text-sm">{content}</p>
  );
};
...
const PostSelectionModal: React.FC<PostSelectionModalProps> = ({
  ...
  onClose,
  onApply,
  title = "Select Posts for Writing Style Analysis",
  description = "Click folder names to browse posts, or click circles to select all posts from that author",
  initialSelectedPosts = { twitter: new Set(), linkedin: new Set() }
}) => {
    ...
    try {
      ...
      try {
        ...
        if (twitterResponse.data.success) {
          ...
          setSavedPosts(prev => ({ ...prev, twitter: twitterPosts }));
        }
      } catch (error) {
      ...
      try {
        ...
        if (linkedinResponse.data.success) {
          ...
          setSavedPosts(prev => ({ ...prev, linkedin: linkedinPosts }));
        }
      } catch (error) {
  ...
  const organizePostsByAuthor = () => {
    ...
    savedPosts.twitter.forEach(post => {
    ...
    savedPosts.linkedin.forEach(post => {
  ...
  // Select all posts from an author
  ...
  // Open a folder (show posts from specific author)
  ...
  const getSelectedPostsContent = () => {
    ...
    selectedPosts.linkedin.forEach(postId => {
      const post = savedPosts.linkedin.find(p => 
  ...
  const handleApply = () => {
    ...
    const counts = {
      twitter: selectedPosts.twitter.size,
      linkedin: selectedPosts.linkedin.size
    };
    
    onApply(content, counts);
    
    toast({
      title: "Posts selected",
      description: `Selected ${counts.twitter + counts.linkedin} posts for analysis`,
    });
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg w-full max-w-6xl max-h-[85vh] flex flex-col shadow-xl">
        {/* Header */}
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              {currentViewMode === 'folders' ? (
                ...
                <>
                  {(() => {
                    ...
                    const areAllPostsSelected = (platform: 'twitter' | 'linkedin', posts: any[]) => {
                      ...
                      return allPostIds.every(id => selectedPosts[platform].has(id));
                    };
                ...
                // Individual Posts View - Show rich post UI
                <>
                  {(() => {
                    ...
                    return (
                      <div>
                        ...
                        {currentOpenFolder.platform === 'twitter' ? (
                          // Twitter Posts with Rich UI - Vertical Layout
                          <div className="space-y-4 max-h-96 overflow-y-auto">
                            {posts.map((post, index) => {
                              const postId = post.id || post._id || post.tweet_id || index.toString();
                              const isSelected = selectedPosts.twitter.has(postId);
                          ...
                          // LinkedIn Posts with Rich UI - Vertical Layout
                          <div className="space-y-4 max-h-96 overflow-y-auto">
                            {posts.map((post, index) => {
                              ...
                              return (
                                ...
                                >
                                  <div 
                                    className={`linkedin-post-card bg-white border border-gray-200 hover:shadow-lg transition-all duration-200 cursor-pointer rounded-lg ${
                                      isSelected ? "ring-2 ring-blue-500" : ""
                                    }`}
        ...
        <div className="border-t p-4 bg-gray-50 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            Selected: {selectedPosts.twitter.size + selectedPosts.linkedin.size} posts
          </div>
          <div className="flex gap-2">
            ...
            <Button 
              onClick={handleApply}
              disabled={selectedPosts.twitter.size + selectedPosts.linkedin.size === 0}
